...
const NAV = [
  ['general', 'General', 'settings', 'How Tomail looks, when it checks for mail, and what it does with your window.'],
  ['accounts', 'Accounts', 'user', 'The mailboxes Tomail is signed in to, their signatures and the addresses they can send from.'],
  ['rules', 'Rules', 'zap', 'File, label or bin new mail automatically as it arrives.'],
  ['snippets', 'Snippets', 'edit', 'Reusable text you can drop into a message by typing a short trigger.'],
  ['ai', 'Local AI', 'star', 'Optional summaries, reply suggestions, drafting in your tone, rewriting and rules from a sentence — all on this computer.'],
  ['security', 'Security & data', 'lock', 'App lock, link cleaning, exporting your mail and achievements.'],
  ['google', 'Advanced', 'sliders', 'Use your own Google API client instead of the one built into Tomail.'],
];
...
export default function SettingsModal({ onClose, accounts, refreshAccounts, toast, info, initialTab, labels = {}, ruleSeed }) {
  ...
  const setAi = (patch) => pref('ai', { ...aiCfg, ...patch });
  ...
  const pullModel = async (m) => { setPull({ status: 'starting' }); try { await flushPrefs(); await window.mail.ai.pull(m); toast(`Downloaded ${m}`); setAi({ model: m }); refreshAi(); } catch (e) { toast(e.message, true); } finally { setPull(null); } };
  ...
  const flushPrefs = useCallback(async () => {
    ...
    try { await window.mail.settings.set({ prefs: patch }); setSavedAt(Date.now()); }
  ...
  useEffect(() => { const k = (e) => { if (e.key === 'Escape') { flushPrefs(); onClose(); } }; document.addEventListener('keydown', k); return () => document.removeEventListener('keydown', k); }, [onClose, flushPrefs]);
  if (!s) return null;
  const save = async (patch) => { const n = await window.mail.settings.set(patch); setS(n); toast('Settings saved'); };
  // Preferences save themselves a moment after you change them — no Save button to forget.
  const pref = (k, v) => {
    setS(cur => ({ ...cur, prefs: { ...cur.prefs, [k]: v } }));
    pendingPrefs.current[k] = v;
    clearTimeout(prefTimer.current);
    prefTimer.current = setTimeout(flushPrefs, 400);
  };
  ...
  return (
    <div className="overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) { flushPrefs(); onClose(); } }}>
      <div className="modal settings">
        <div className="mh">Settings<button className="x" onClick={() => { flushPrefs(); onClose(); }}>✕</button></div>
        <div className="mb">
          <nav className="snav">
            {NAV.map(([id, label, icon]) => <button key={id} className={tab === id ? 'on' : ''} onClick={() => setTab(id)}><Icon name={icon} size={14} /> {label}</button>)}
          </nav>
          <div className="spane">
          <h2 className="shead">{cur[1]}</h2>
          <p className="sdesc">{cur[3]}</p>
          {tab === 'general' && <>
            <div className="sgroup">Appearance</div>
            <div className="frow"><label>Theme</label><div style={{ display: 'flex', gap: 4 }}>{[['system', 'System'], ['light', 'Light'], ['dark', 'Dark']].map(([v, n]) => <button key={v} className={s.prefs.theme === v || (!s.prefs.theme && v === 'system') ? 'primary' : ''} onClick={() => { pref('theme', v); document.documentElement.dataset.theme = v === 'system' ? '' : v; document.documentElement.classList.toggle('dark', v === 'dark' || (v === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches)); }}>{v === 'light' ? <Icon name="sun" size={12} /> : v === 'dark' ? <Icon name="moon" size={12} /> : null} {n}</button>)}</div></div>

            <div className="sgroup">Checking for mail</div>
            <div className="frow"><label>While Tomail is active</label><div>every <input type="number" min="10" style={{ width: 70 }} value={s.prefs.fastPollSec ?? 20} onChange={e => pref('fastPollSec', Number(e.target.value))} /> seconds</div></div>
            <div className="frow"><label>In the background</label><div>every <input type="number" min="15" style={{ width: 70 }} value={s.prefs.syncIntervalSec} onChange={e => pref('syncIntervalSec', Number(e.target.value))} /> seconds</div></div>
            <div className="frow"><label></label><small>IMAP accounts are pushed to instantly by the server as well (IMAP IDLE), and a first sync of a large mailbox keeps checking for new mail while it runs.</small></div>

            <div className="sgroup">Notifications</div>
            <div className="frow"><label>New mail</label><label><input type="checkbox" checked={s.prefs.notifications !== false} onChange={e => pref('notifications', e.target.checked)} /> <Icon name="bell" size={12} /> show a system notification for new inbox mail</label></div>
            ...
            <div className="sgroup">Keeping Tomail running</div>
            <div className="frow"><label>Tray icon</label><label title="Tomail sits in the system tray so new mail still reaches you"><input type="checkbox" checked={s.prefs.tray !== false} onChange={e => pref('tray', e.target.checked)} /> show a tray icon</label></div>
            <div className="frow"><label></label><label style={{ opacity: s.prefs.tray === false ? 0.5 : 1 }}><input type="checkbox" disabled={s.prefs.tray === false} checked={s.prefs.closeToTray !== false} onChange={e => pref('closeToTray', e.target.checked)} /> closing the window leaves Tomail in the tray instead of quitting</label></div>
            <div className="frow"><label></label><label><input type="checkbox" checked={!!s.prefs.startAtLogin} onChange={e => pref('startAtLogin', e.target.checked)} /> start Tomail when I log in (hidden, in the tray)</label></div>
            ...
            {(s.prefs.imageSenders || []).length > 0 && <div className="frow"><label>Always from</label><div className="chips inline">{s.prefs.imageSenders.map(e => <span key={e} className="chip">{e}<button title="Stop loading images from this sender automatically" onClick={() => pref('imageSenders', s.prefs.imageSenders.filter(x => x !== e))}>✕</button></span>)}</div></div>}
            ...
            <div className="sgroup">Storage</div>
            ...
            <div className="frow"><label>Local database</label><div><small>{dbInfo ? `${(dbInfo.size / 1048576).toFixed(0)} MB · ${dbInfo.messages.toLocaleString()} messages, ${dbInfo.bodies.toLocaleString()} bodies cached · compacted ${dbInfo.lastVacuum ? new Date(dbInfo.lastVacuum).toLocaleDateString('en-GB') : 'never'}` : '…'}</small> <button onClick={async () => { await flushPrefs(); const r = await window.mail.app.compactDb(); toast(`Compacted${r.pruned ? `, cleared ${r.pruned} old bodies` : ''}`); window.mail.app.dbInfo().then(setDbInfo); }}>Compact now</button></div></div>
          ...
          {tab === 'ai' && <div>
            <p className="muted">Nothing is sent to Tomail or anyone else. It needs <a href="#" onClick={e => { e.preventDefault(); window.mail.shell.openExternal('https://ollama.com/download'); }}>Ollama</a>, a free app that runs language models on your own machine: install it, then pick a model below.</p>
            <div className="frow"><label>Enable AI features</label><label><input type="checkbox" checked={!!aiCfg.enabled} onChange={e => setAi({ enabled: e.target.checked })} /> show the AI buttons in messages, compose and rules</label></div>
            ...
            <div className="frow"><label>Learn my tone</label><label><input type="checkbox" checked={aiCfg.styleLearning !== false} onChange={e => setAi({ styleLearning: e.target.checked })} /> show the model a few of your recent sent messages when drafting, so replies sound like you</label></div>
            <details style={{ marginTop: 8 }}><summary className="muted">Advanced: other local servers</summary>
              ...
              <div className="frow"><label></label><button className="primary" onClick={async () => { await flushPrefs(); toast('AI settings saved'); refreshAi(); }}>Save</button></div>
          ...
          {tab === 'security' && <div>
            ...
            <h3>Achievements</h3>
            <div className="frow"><label>Show achievements</label><label><input type="checkbox" checked={s.prefs.achievements !== false} onChange={e => pref('achievements', e.target.checked)} /> little toasts for milestones (inbox zero, 100 archived…)</label></div>
        ...
        <div className="mf">
          <span className="hint">{savedAt ? 'Saved' : 'Changes are saved as you make them'}</span>
          <span className="spacer" />
          <button className="primary" onClick={() => { flushPrefs(); onClose(); }}>Done</button>
        </div>
      </div>
    </div>
  );
